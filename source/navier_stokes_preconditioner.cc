...
public:
  ...
  std::size_t
  memory_consumption_cell() const
  {
    return sizeof(this);
  }

  std::size_t
  memory_consumption_mat() const
  {
    ...
    if (use_trilinos_matrix)
    ...
    else
      {
        ...
        for (unsigned int i = 0; i < n_constraints; ++i)
          {
            schur_complement_tmp_values[i] =
              src.local_element(constraints_schur_complement_only[i]);
            const_cast<LinearAlgebra::distributed::Vector<double> &>(src).local_element(
              constraints_schur_complement_only[i]) = 0;
          }
        ...
        for (unsigned int i = 0; i < n_constraints; ++i)
          {
            const_cast<LinearAlgebra::distributed::Vector<double> &>(src).local_element(
              constraints_schur_complement_only[i]) = schur_complement_tmp_values[i];
...
public:
             ...
             const std::vector<std::vector<double>> &coordinates)
  {
    ...
    parameter_list.set("repartition: Zoltan dimensions",
                       coordinates.size() > 0 ? (int)coordinates.size() : 3);
    ...
    preconditioner =
      std::make_unique<ML_Epetra::MultiLevelPreconditioner>(matrix, parameter_list);
  }
...
private:
  std::unique_ptr<ML_Epetra::MultiLevelPreconditioner> preconditioner;
};
  ...
  TimerOutput                                           &timer) const
{
  ...
  LinearAlgebra::distributed::Vector<double> solution_u_copy;
  {
    ...
    solution_update = 0;
    SolverGMRES<LinearAlgebra::distributed::Vector<double>> solver(
      solver_control,
    ...
    if (parameters.precondition_velocity == FlowParameters::u_ilu)
      ...
      solver.solve(*uu_amg_mat,
                   solution_update.block(0),
                   system_rhs.block(0),
                   *uu_ilu_scalar);
    else
...

  {
    ...
    SolverControl solver_control(
      1000,
      0.1 * parameters.time_step_size_start /
        std::min(parameters.density, parameters.density + parameters.density_diff) *
    ...
    if (pp_mass == 0)
      solver2.solve(pressure_mass,
                    solution_update.block(1),
                    system_rhs.block(1),
            ...
            if (locally_owned.is_element(local_dof_indices[i]))
              {
                const unsigned int index =
                  locally_owned.index_within_set(local_dof_indices[i]);
                for (unsigned int d = 0; d < dim; ++d)
...
adaflo::NavierStokesPreconditioner<dim>::compute()
{
  ...
  if (parameters.precondition_velocity == FlowParameters::u_ilu)
    {
      if (uu_ilu.get() == 0)
        uu_ilu = std::make_shared<TrilinosWrappers::PreconditionILU>();
    ...
    {
      uu_amg = std::make_shared<Precondition_LinML>();
  ...
  if (parameters.density > 0)
    {
      pp_poisson_mat =
        std::make_shared<PressurePoissonMatrix<dim>>(*matrix,
                                                     matrix_p,
                                                     parameters.augmented_taylor_hood ||
                                                       matrix->pressure_degree() == 1,
                                                     constraints_schur_complement_only);
      pp_poisson = std::make_shared<Precondition_LinML>();
  ...
  if (parameters.augmented_taylor_hood == true)
    {
      ...
      pp_mass             = std::make_shared<TrilinosWrappers::PreconditionAMG>();
    ...
    {
      pp_mass = std::make_shared<TrilinosWrappers::PreconditionILU>();
  ...
  const AffineConstraints<double> &constraints_p)
{
  ...
  if (parameters.precondition_velocity == FlowParameters::u_ilu_scalar)
    {
      ...
      for (unsigned int d = 0; d < dim; ++d)
        if (flow_algorithm.boundary->periodic_boundaries[d] !=
          ...
          {
            const types::boundary_id in =
              flow_algorithm.boundary->periodic_boundaries[d].first;
            const types::boundary_id out =
              flow_algorithm.boundary->periodic_boundaries[d].second;
            DoFTools::make_periodicity_constraints(
              dof_handler_u_scalar, in, out, d, constraints_u_scalar);
          }
      ...
      for (typename std::map<types::boundary_id,
             ...
             flow_algorithm.boundary->dirichlet_conditions_u.begin();
           it != flow_algorithm.boundary->dirichlet_conditions_u.end();
           ++it)
      ...
      for (typename std::set<types::boundary_id>::const_iterator it =
           ...
           it != flow_algorithm.boundary->no_slip.end();
           ++it)
  ...
  if (!flow_algorithm.boundary->open_conditions_p.empty() &&
      !parameters.augmented_taylor_hood)
    {
      ...
      for (typename std::map<types::boundary_id,
                             std::shared_ptr<Function<dim>>>::const_iterator it =
             flow_algorithm.boundary->open_conditions_p.begin();
           it != flow_algorithm.boundary->open_conditions_p.end();
           ++it)
        {
          homogeneous_dirichlet[it->first] = &zero_func;
        }
  ...
  if (!flow_algorithm.boundary->pressure_fix.empty())
    {
      ...
      for (typename std::map<types::boundary_id, std::shared_ptr<Function<dim>>>::iterator
             it = flow_algorithm.boundary->pressure_fix.begin();
           it != flow_algorithm.boundary->pressure_fix.end();
           ++it)
        {
      ...
      for (std::map<types::global_dof_index, double>::iterator it =
      ...
      const types::global_dof_index min_index =
        -Utilities::MPI::max(min_local_index, triangulation.get_mpi_communicator());
    ...
    if (parameters.precondition_velocity == FlowParameters::u_amg ||
        parameters.precondition_velocity == FlowParameters::u_ilu)
      {
        ...
        DoFTools::make_sparsity_pattern(
          dof_handler_u, coupling, csp, constraints_u, false, this_mpi_process);
      }
    else if (parameters.precondition_velocity == FlowParameters::u_ilu_scalar)
      {
        ...
        std::vector<types::global_dof_index> dof_indices(
          ...
          vdof_indices(fe_u.dofs_per_cell);
        for (typename DoFHandler<dim>::active_cell_iterator
               cell  = dof_handler_u_scalar.begin_active(),
               vcell = dof_handler_u.begin_active();
             cell != dof_handler_u_scalar.end();
             ++cell, ++vcell)
          if (cell->is_locally_owned())
            {
                ...
                if (reduced_owned.is_element(dof_indices[i]))
                  {
                    const unsigned int local_index =
                      reduced_owned.index_within_set(dof_indices[i]);
                    for (unsigned int d = 0; d < dim; ++d)
                      {
          ...
          if (cell->is_locally_owned())
            {
              ...
              for (unsigned int i = 0; i < dof_to_lin_u.size(); ++i)
                for (unsigned int d = 0; d < dim; ++d)
                  {
                    for (unsigned int k = 0; k < GeometryInfo<dim>::vertices_per_cell;
                         ++k)
  ...
  // Build sparsity pattern for the pressure
  {
    ...
    if (parameters.precondition_velocity != FlowParameters::u_amg_linear ||
        parameters.augmented_taylor_hood)
      DoFTools::make_sparsity_pattern(
        dof_handler_p, csp, constraints_schur_complement, false, this_mpi_process);
    else
      {
              ...
              for (unsigned int i = 0; i < dof_to_lin_p.size(); ++i)
                {
                  ...
                  constraints_schur_complement.add_entries_local_to_global(
                    linear_dof_indices, csp, false);
                }
            }
      }
    csp.compress();
                    ...
                    if (cell->face(face)->has_children())
                      for (unsigned int i = 0;
                           ...
                           ++i)
                        {
                          ...
                          constraints_schur_complement.add_entries_local_to_global(
                            const_dof_index, local_dof_indices, sparsity, false);
                          constraints_schur_complement.add_entries_local_to_global(
                            local_dof_indices, const_dof_index, sparsity, false);
                        }
                    else
                      {
  ...
  if (parameters.augmented_taylor_hood && parameters.density_diff != 0.)
    {
      ...
      for (typename Triangulation<dim>::active_cell_iterator cell =
        ...
        if (!cell->is_artificial())
          max_index_per_cell[cell->level()] =
            std::max(max_index_per_cell[cell->level()], (unsigned int)cell->index());
...
namespace adaflo::AssemblyData
{
  ...
  struct Preconditioner
  {
    ...
    Preconditioner()
      : fe_values_u(MappingQGeneric<dim>(1),
                    FE_Q<dim>(1),
                    QGauss<dim>(2),
                    update_values | update_gradients | update_JxW_values)
      , fe_values_p(MappingQGeneric<dim>(1),
                    FE_Q<dim>(1),
                    fe_values_u.get_quadrature(),
                    update_values | update_gradients | update_JxW_values)
      , fe_face_values_p(MappingQGeneric<dim>(1),
                         FE_Q<dim>(1),
                         QGauss<dim - 1>(2),
                         update_values | update_gradients | update_JxW_values |
                           update_normal_vectors)
      , fe_subface_values_p(MappingQGeneric<dim>(1),
                            FE_Q<dim>(1),
  ...
  Preconditioner<dim>::Preconditioner(
    ...
    , velocity_matrix(fe_u.dofs_per_cell, fe_u.dofs_per_cell)
    , pressure_matrix(fe_p.dofs_per_cell, fe_p.dofs_per_cell)
    , pressure_mass_matrix(fe_p.dofs_per_cell, fe_p.dofs_per_cell)
    ...
    , pressure_laplace_part_matrix(fe_values_p.n_quadrature_points * dim,
                                   fe_p.dofs_per_cell)
    , pressure_mass_part_matrix(fe_values_p.n_quadrature_points, fe_p.dofs_per_cell)
    , velocity_matrix_permuted(fe_u.base_element(0).dofs_per_cell, fe_u.dofs_per_cell)
...
namespace
{
    ...
    FullMatrix<double>                                      &matrix_2)
  {
    ...
    const unsigned int degsq =
      fe_face_values.get_fe().degree * (fe_face_values.get_fe().degree + 1);
                         ...
                         FullMatrix<double>          &matrix)
  {
    ...
    const double penalty =
      degsq / fe_face_values.get_cell()->extent_in_direction(normal1);
  ...
  const std::pair<unsigned int, unsigned int> &cell_range)
{
  ...
  for (unsigned int mcell = cell_range.first; mcell < cell_range.second; ++mcell)
    {
      for (unsigned int vec = 0; vec < matrix_free.n_active_entries_per_cell_batch(mcell);
           ++vec)
        {
                  ...
                  for (unsigned int i = 0; i < dofs_per_u_component; ++i)
                    {
                      const Tensor<1, dim> &phi_grad_iq =
                        data.fe_values_u.shape_grad(i, q);
                      ...
                      for (unsigned int d = 0; d < dim; ++d)
                        {
                          ...
                          if (scalar_ilu)
                            data.velocity_phi_and_grad_matrix((dim + 1) * q + d, i) =
                              phi_grad_iq[d];
                          else
                            {
                              for (unsigned int e = 0; e < dim; ++e)
                                data.velocity_phi_and_grad_matrix((dim + 1) * q + d,
                                                                  i * dim + e) =
                                  phi_grad_iq[d];
                              data.velocity_phi_and_grad_matrix((dim + 1) * q + d,
                                                                i * dim + d) +=
                      ...
                      if (linearized != 0)
                        {
                          for (unsigned int d = 0; d < dim; ++d)
                            phi_and_grad +=
                              tau * phi_grad_iq[d] * linearized[q].first[d][vec];
                          if (!scalar_ilu)
                            for (unsigned int d = 0; d < dim; ++d)
              ...
              if (!scalar_ilu)
                {
                  ...
                  constraints_u.distribute_local_to_global(data.velocity_matrix,
                                                           data.local_dof_indices_u,
                                                           matrix_u);
                }
              else
                {
                  typename DoFHandler<dim>::active_cell_iterator cell_u_scalar(
                    &triangulation,
                    cell_u->level(),
                    cell_u->index(),
                    &dof_handler_u_scalar);
                  ...
                  constraints_u_scalar.distribute_local_to_global(
                    data.velocity_matrix_permuted,
                    data.local_dof_indices_u_scalar,
                    matrix_u);
                }
            }
          ...
          // Now treat the case with preconditioner based on linear elements
          else
            {
              // start with velocity
              {
                ...
                const std::vector<std::vector<unsigned int>> &dof_to_lin_u =
                  integration_helper.dof_to_lin_u;
                const std::vector<std::vector<unsigned int>> &quad_to_lin_u =
                  integration_helper.quad_to_lin_u;
                    ...
                    for (unsigned int d = 0; d < dim; ++d)
                      {
                        ...
                        for (unsigned int k = 0; k < n_dofs; ++k)
                          data.linear_dof_indices[k] =
                            data.local_dof_indices_u
                              [local_ordering_u[d][dof_to_lin_u[sub][k]]];

                        for (unsigned int q = 0; q < n_dofs; ++q)
                          {
                            Tensor<2, dim> jac = data.fe_values_lin_u.inverse_jacobian(
                              quad_to_lin_u[sub][q]);
                                ...
                                if (linearized != 0)
                                  {
                                    ...
                                    phi_and_grad +=
                                      tau * integration_helper.values_unit_cell[i][q] *
                        ...
                        constraints_u.distribute_local_to_global(data.linear_matrix,
                                                                 data.linear_dof_indices,
                                                                 matrix_u);
                      }
                  }
              }
              ...
              if (!parameters.augmented_taylor_hood)
                {
                  ...
                  const std::vector<std::vector<unsigned int>> &dof_to_lin_p =
                    integration_helper.dof_to_lin_p;
                  const std::vector<std::vector<unsigned int>> &quad_to_lin_p =
                    integration_helper.quad_to_lin_p;
                  ...
                  for (unsigned int sub = 0; sub < dof_to_lin_p.size(); ++sub)
                    {
                      ...
                      if (parameters.physical_type ==
                        ...
                        rho_weight = 1.;
                      double actual_nu =
                        ((use_variable_parameters ? viscosities[0][vec] :
                                                    parameters.viscosity) +
                         parameters.tau_grad_div);
                      if (parameters.physical_type ==
                            FlowParameters::incompressible_stationary ||
                      ...
                      for (unsigned int q = 0; q < n_dofs; ++q)
                        {
                          Tensor<2, dim> jac =
                            data.fe_values_lin_p.inverse_jacobian(quad_to_lin_p[sub][q]);
                          ...
                          for (unsigned int i = 0; i < n_dofs; ++i)
                            {
                              data_for_val[i][q] =
                                weight_p_nu * integration_helper.values_unit_cell[i][q];
                              if (rho_weight <= 0)
                                continue;
                      ...
                      if (parameters.density > 0)
                        {
                          ...
                          constraints_schur_complement.distribute_local_to_global(
                            data.linear_matrix, data.linear_dof_indices, matrix_p);
                        }
                      ...
                      if (parameters.precondition_pressure == FlowParameters::p_mass_ilu)
                        {
                          ...
                          constraints_schur_complement.distribute_local_to_global(
                            data.linear_matrix, data.linear_dof_indices, mass_matrix_p);
                        }
                    }
                }
            }
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  ...
                  if (parameters.physical_type ==
                      FlowParameters::incompressible_stationary)
                    rho_weight = 1.;
                  double actual_nu = ((use_variable_parameters ? viscosities[q][vec] :
                                                                 parameters.viscosity) +
                                      parameters.tau_grad_div);
                  if (parameters.physical_type ==
                        FlowParameters::incompressible_stationary ||
                  ...
                  for (unsigned int i = 0; i < dofs_per_cell_p; ++i)
                    {
                      const Tensor<1, dim> &phi_grad_iq =
                        data.fe_values_p.shape_grad(i, q);
                      if (rho_weight > 0)
                        for (unsigned int d = 0; d < dim; ++d)
                          {
                      ...
                      if (parameters.precondition_pressure ==
                            FlowParameters::p_mass_ilu ||
                          parameters.augmented_taylor_hood)
                        data.pressure_mass_part_matrix(q, i) =
                          weight_p_nuf * data.fe_values_p.shape_value(i, q);
                    }
                }
              cell_p->get_dof_indices(data.local_dof_indices_p);
              if (parameters.density > 0)
                {
                  ...
                  data.pressure_laplace_part_matrix.Tmmult(
                    data.pressure_matrix, data.pressure_laplace_part_matrix);
                               ...
                               ++subf)
                            {
                              ...
                              const double density =
                                neighbor_child->is_locally_owned() ?
                                  get_face_average_density(neighbor_child,
                                                           cell->neighbor_face_no(f)) :
                                  get_face_average_density(cell, f);
                              compute_ip_matrix_q_dg0(data.fe_subface_values_p,
                                                      f,
                                                      neighbor_child,
                              ...
                              neighbor_child->get_dof_indices(
                                data.local_dof_indices_p_neigh);
                              for (unsigned int i = 0; i < dofs_per_cell_p; ++i)
                              ...
                              data.local_dof_indices_p_dg0[0] =
                                data.local_dof_indices_p_neigh.back();
                              constraints_schur_complement.distribute_local_to_global(
                                data.pressure_ip_matrix_1,
                                data.local_dof_indices_p_dg0,
                                data.local_dof_indices_p,
                                matrix_p);
                              constraints_schur_complement.distribute_local_to_global(
                                data.pressure_ip_matrix_2,
                                data.local_dof_indices_p,
                  ...
                  constraints_schur_complement.distribute_local_to_global(
                    data.pressure_matrix, data.local_dof_indices_p, matrix_p);
                }
              if (parameters.precondition_pressure == FlowParameters::p_mass_ilu ||
                  parameters.augmented_taylor_hood)
                {
                  data.pressure_mass_part_matrix.Tmmult(data.pressure_mass_matrix,
                                                        data.pressure_mass_part_matrix);
                  constraints_schur_complement.distribute_local_to_global(
                    data.pressure_mass_matrix, data.local_dof_indices_p, mass_matrix_p);
                }
            }
        }
      if (linearized != 0)
...
adaflo::NavierStokesPreconditioner<dim>::assemble_matrices()
{
  ...
  matrix->get_matrix_free().cell_loop(
      ...
      for (unsigned int i = 0; i < index.n_elements(); ++i)
        {
          ...
          if (constraints_schur_complement.is_constrained(idx) &&
              !std::binary_search(constraints_schur_complement_only.begin(),
                                  constraints_schur_complement_only.end(),
                                  i))
  ...
  const FiniteElement<dim> &fe_u)
{
  local_ordering_u.resize(dim,
                          std::vector<unsigned int>(fe_u.base_element(0).dofs_per_cell));
  ...
  const FiniteElement<dim> &fe_p)
{
  ...
  QGauss<dim> quadrature_linear(2);
  {
    ...
    for (unsigned int i = 0; i < n_dofs; ++i)
      for (unsigned int q = 0; q < n_dofs; ++q)
        {
          values_unit_cell[i][q] = fe_q.shape_value(i, quadrature_linear.point(q));
          grads_unit_cell[i][q]  = fe_q.shape_grad(i, quadrature_linear.point(q));
        }
  }
  ...
  quadrature_sub_u            = std::make_unique<QIterated<dim>>(QGauss<1>(2), degree_u);
  quadrature_sub_p            = std::make_unique<QIterated<dim>>(QGauss<1>(2), degree_p);
  ...
  std::ostream &stream) const
{
  stream << "| Preconditioner matrices: "
         << 1e-6 *
              double(matrix_u.memory_consumption() + matrix_p.memory_consumption() +
  ...
  if (uu_ilu.get() != 0 || uu_ilu_scalar.get() != 0)
    stream << "| ILU preconditioner for uu block: "
           << 1e-6 * double(matrix_u.memory_consumption()) << " MB\n";
  else if (uu_amg.get() != 0)
    stream << "| AMG preconditioner for uu block: "
           << 1e-6 * double(uu_amg->memory_consumption()) << " MB\n";
  if (pp_poisson.get() != 0)
    stream << "| AMG preconditioner for pp block: "
           << 1e-6 * double(pp_poisson->memory_consumption()) << " MB\n";
}
...
adaflo::NavierStokesPreconditioner<dim>::get_timer_statistics() const
{
  ...
  for (unsigned int i = 0; i < 5; ++i)
    {
      data.first[i]           = Utilities::MPI::min_max_avg(precond_timer.second[i],
