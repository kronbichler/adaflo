          ...
          for (unsigned int q=0; q<ls_values.n_q_points; ++q)
            {
              ...
              heaviside_val = std::min(make_vectorized_array(1.),
                                       std::max(make_vectorized_array(0.),
                                                heaviside_val));
              densities[q] = this->parameters.density +
                             this->parameters.density_diff * heaviside_val;
      ...
      for (unsigned int i=0; i<pre_values.dofs_per_cell; ++i)
        {
          ...
          for (unsigned int j=0; j<ls_values.dofs_per_cell; ++j)
            projected_value += interpolation_concentration_pressure(i, j) *
                                       ...
                                       const std::pair<unsigned int,unsigned int> &cell_range) const
{
  ...
  const double factor_mobility = inv_time_weight *
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      VectorizedArray<double> *w_values =
        &evaluated_phi[cell*c_values.n_q_points];
      ...
      for (unsigned int q=0; q<c_values.n_q_points; ++q)
        {
          ...
          const Tensor<1,dim,VectorizedArray<double> > w_grad = phi_values.get_gradient(q);
          Tensor<1,dim,VectorizedArray<double> > velocity =
          ...
          c_values.submit_gradient(make_vectorized_array(factor_mobility) *
                                   w_grad, q);

          phi_values.submit_value(w_val - factor_2 * c_val * (c_val*c_val-1.),
                                  q);
          phi_values.submit_gradient(make_vectorized_array(-factor_4) *
                                     c_grad, q);
        }
  ...
  for (unsigned int f=0; f<n_faces; ++f)
    {
      ...
      this->constraints.get_dof_values(src.block(0), indices, local_dofs.begin(),
                                       local_dofs.end());
      for (unsigned int q=0; q<n_face_q_points; ++q)
        {
          ...
          if (operation == 1)
            {
              c_values[q] = sum;
              evaluated[q] = this->parameters.contact_angle * this->parameters.surface_tension * 0.75 * (sum * sum - 1.) * JxW[q];
            }
          else
            evaluated[q] = this->parameters.contact_angle * this->parameters.surface_tension * 1.5 * sum * c_values[q] * JxW[q];
        }
      for (unsigned int i=0; i<this->fe->dofs_per_face; ++i)
        {
          double sum = 0;
      ...
      this->constraints_curvature.distribute_local_to_global(local_dofs.begin(), local_dofs.end(),
                                                             indices, dst.block(1));
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&PhaseFieldSolver<dim>::template         \
                               local_residual<c_degree,u_degree>,       \
                               this, this->system_rhs, this->solution)
