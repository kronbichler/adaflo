...
template <int dim>
void LevelSetOKZMatrixSolver<dim>::initialize_data_structures ()
{
  system_matrix.clear();
  ...
  system_matrix.reinit(this->dof_handler.locally_owned_dofs(), dsp,
                       this->triangulation.get_communicator(), true);
}



template <int dim>
void LevelSetOKZMatrixSolver<dim>::compute_force ()
{
  compute_heaviside();
  compute_curvature();
  ...
  FEValues<dim> fe_values (this->mapping, this->navier_stokes.get_fe_u(),
  ...
  {
    FEValues<dim> interpolate_for_p (*this->fe, this->navier_stokes.get_fe_p().
  ...
  FEValues<dim> ls_values (this->mapping,
                           *this->fe, this->matrix_free.get_quadrature(0),
  ...
  for (unsigned int mcell = 0; mcell < this->matrix_free.n_macro_cells();
       ++mcell)
    for (unsigned int vec=0; vec<this->matrix_free.n_components_filled(mcell); ++vec)
      {
        local_rhs = 0;

        VectorizedArray<double> *densities = use_variable_parameters ?
                                             this->navier_stokes.get_matrix().begin_densities(mcell) : 0;
        VectorizedArray<double> *viscosities = use_variable_parameters ?
                                               this->navier_stokes.get_matrix().begin_viscosities(mcell) : 0;
        typename DoFHandler<dim>::active_cell_iterator
        cell = this->matrix_free.get_cell_iterator (mcell, vec, 0),
        ls_cell (&this->triangulation, cell->level(), cell->index(),
                 &this->dof_handler);
        ...
        ls_values.get_function_values (this->parameters.surface_tension_from_heaviside ?
                                       this->heaviside :
                                       this->solution.block(0), c_values);
        if (this->parameters.interpolate_grad_onto_pressure == false)
          ls_values.get_function_gradients (this->parameters.surface_tension_from_heaviside ?
                                            this->heaviside :
                                            this->solution.block(0), c_gradients);
        else
          {
            ls_cell->get_interpolated_dof_values (this->parameters.surface_tension_from_heaviside ?
                                                  this->heaviside :
                                                  this->solution.block(0), c_val);
        ...
        for (unsigned int q=0; q<n_q_points; ++q)
          {
            ...
            Tensor<1,dim> grad_c = ( this->parameters.surface_tension_from_heaviside ? 1. : 0.5 ) *
                                   (tau1 * c_gradients[q] + tau2 * c_gradients_old[q]);

            Tensor<1,dim> force =  (grad_c *
                                    curvature * this->parameters.surface_tension);
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)
              {
                local_rhs(i) += (force * fe_values[velocities].value(i,q)
                                ) * fe_values.JxW(q);
              }
          }
...
LevelSetOKZMatrixSolver<dim>::advance_concentration ()
{
  ...
  FEValues<dim> fe_values (this->mapping, *this->fe, this->matrix_free.get_quadrature(2),
                           update_values   | update_gradients |
                           update_inverse_jacobians | update_JxW_values);

  FEValues<dim> ns_values (this->mapping, this->navier_stokes.get_fe_u(),
                           fe_values.get_quadrature(), update_values);
  ...
  ns_cell = this->navier_stokes.get_dof_handler_u().begin_active();
    ...
    if (cell->is_locally_owned())
      {
        ...
        ns_values[velocities].get_function_values (this->navier_stokes.solution.block(0),
                                                   velocity_values);
        ns_values[velocities].get_function_values (this->navier_stokes.solution_old.block(0),
                                                   old_velocity_values);
        fe_values.get_function_values (this->solution.block(0),
                                       cur_values);
        fe_values.get_function_gradients (this->solution.block(0),
                                          cur_gradients);
        fe_values.get_function_values (this->solution_old.block(0),
                                       old_values);
        fe_values.get_function_gradients (this->solution_old.block(0),
                                          old_gradients);
        if (this->time_stepping.step_no() > 1)
          {
            fe_values.get_function_values (this->solution_old_old.block(0),
                                           old_old_values);
            ns_values[velocities].get_function_values (this->navier_stokes.solution_old_old.block(0),
        ...
        const double viscosity = this->parameters.convection_stabilization ?
                                 compute_viscosity (old_values, old_old_values,
                                                    old_gradients, old_old_gradients,
                                                    old_velocity_values, old_old_velocity_values,
                                                    step_size_old, global_max_velocity, global_conc_range,
                                                    this->global_omega_diameter, cell->diameter())
                                 : 0.;
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                  {
                    ...
                    cell_rhs(i) += (fe_values.shape_value(i,q)*
                                    (old_value
                                     -
                                     this->time_stepping.weight()*cur_values[q])
                                    +
                                    u_nabla_eta * cur_values[q]
                                    /*-
                                      viscosity * fe_values.shape_grad(i,q) * cur_gradients[q]*/
                                   )*fe_values.JxW(q);
                  };
              }
          }
        ...
        this->constraints.distribute_local_to_global (cell_matrix,
                                                      local_dof_indices,
                                                      system_matrix);
        this->constraints.distribute_local_to_global (cell_rhs, local_dof_indices,
                                                      rhs);
      };
...
LevelSetOKZMatrixSolver<dim>::compute_normal (const bool fast_computation)
{
  ...
  const double damping = (this->parameters.use_anisotropic_refinement == true ? 16. : 4. )
                         * numbers::NumberTraits<double>::abs_square(this->epsilon_used / this->parameters.epsilon);
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)
              {
                ...
                for (unsigned int d=0; d<dim; ++d)
                  cell_rhs[d](i) += (fe_values.shape_value(i,q) *
                                     ls_gradient[q][d]
                                    )* fe_values.JxW (q);
              }
          }
...
LevelSetOKZMatrixSolver<dim>::compute_curvature (const bool )
{
  ...
  const double damping = (this->parameters.use_anisotropic_refinement == true ? 4. : 0.5 )
                         * numbers::NumberTraits<double>::abs_square(this->epsilon_used / this->parameters.epsilon);
...
// elements now
template <int dim>
void LevelSetOKZMatrixSolver<dim>::compute_heaviside ()
{
...
template <int dim>
void LevelSetOKZMatrixSolver<dim>::reinitialize (const unsigned int stab_steps,
  ...
  for (unsigned int tau=0; tau<diff_steps+stab_steps; tau++)
    {
      ...
      for (typename DoFHandler<dim>::active_cell_iterator
           cell=this->dof_handler.begin_active ();
           cell!=this->dof_handler.end (); ++cell)
        if (cell->is_locally_owned())
          {
            cell_matrix = 0;
            ...
            double diffusion = std::max(this->epsilon_used,
                                        cell_diameter);
            ...
            if (!diffuse_only)
              {
                for (unsigned int d=0; d<dim; ++d)
                  {
                    cell->get_dof_values(this->normal_vector_field.block(d),
                                         cell_rhs);
                    for (unsigned int q=0; q<n_q_points; ++q)
                      {
                        double val = 0;
                ...
                double norm = 0;
                for (unsigned int q=0; q<n_q_points; ++q)
                  norm += std::fabs(cur_solution_values[q] * cur_solution_values[q]
                                    - 1.);
                if (norm < 1e-10)
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
              {
                ...
                Tensor<1,dim> normal = diffuse_only ? Tensor<1,dim>() : normal_values[q];
                if (!disable_convection && !diffuse_only)
                  normal /= normal.norm();

                const double diff_val = diffusion * normal *
                                        cur_solution_gradients[q];
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                  {
                    ...
                    if (!diffuse_only && !disable_convection)
                      {
                        ...
                        cell_rhs(i) += (n_nabla_eta * (0.5 *
                                                       (1. - cur_solution_values[q] *
                                                        cur_solution_values[q]) -
                                                       diff_val)
                                       ) * fe_values.JxW (q);
                      }
                    // just diffusion
