...
void FlowParameters::check_for_file (const std::string &parameter_filename,
...
void FlowParameters::declare_parameters (ParameterHandler &prm)
{
  ...
  prm.declare_entry ("adaptive refinements","0",Patterns::Integer(),
                     ...
                     "applications.");
  prm.declare_entry ("velocity degree","2",Patterns::Integer(),
                     "Sets the degree for velocity. Pressure degree is "
                     "velocity degree minus one. Currently implemented for "
                     "orders 2 to 6");
  prm.declare_entry ("augmented Taylor-Hood elements","0",Patterns::Integer(),
  ...
  prm.declare_entry ("density","1.",Patterns::Double(),
                     "Defines the fluid density");
  prm.declare_entry ("physical type", "incompressible",
                     Patterns::Selection("incompressible|incompressible stationary|stokes"),
  ...
  prm.declare_entry ("NL max iterations","10",Patterns::Integer(),
                     "Defines the maximum number of nonlinear Newton "
                     "iterations.");
  prm.declare_entry ("NL tolerance","1e-6",Patterns::Double(),
                     "Defines the tolerance in the residual l2 norm in "
                     "the nonlinear Newton iteration.");
  prm.declare_entry ("linearization scheme","coupled implicit Newton",
                     Patterns::Selection("coupled implicit Newton|coupled implicit Picard|coupled velocity semi-implicit|coupled velocity explicit|projection"),
  ...
  prm.declare_entry ("tau grad div","0.",Patterns::Double(),
  ...
  prm.declare_entry ("lin max iterations","500",Patterns::Integer(),
                     "Maximum number of linear iterations");
  prm.declare_entry ("lin tolerance","1.e-3",Patterns::Double(),
                     "Tolerance for the linear solver");
  prm.declare_entry ("lin relative tolerance","1",Patterns::Integer(),
                     "Sets whether the residual for the linear solver "
                     "should be measured relative to the nonlinear residual "
                     "(recommended option).");
  prm.declare_entry ("lin velocity preconditioner","amg linear",
                     ...
                     "somewhat worse.");
  prm.declare_entry ("lin pressure mass preconditioner","ilu",
  ...
  prm.declare_entry ("lin its before inner solvers","50",Patterns::Integer(),
  ...
  prm.declare_entry ("density difference","0.",Patterns::Double(),
  ...
  prm.declare_entry ("viscosity difference","0.",Patterns::Double(),
                     "absolute difference in viscosity compared to fluid 1");

  prm.declare_entry ("surface tension","1.",Patterns::Double(),
                     "surface tension coefficient");
  prm.declare_entry ("epsilon","1",Patterns::Double(),
  ...
  prm.declare_entry ("localize surface tension","1",Patterns::Integer(),
                     ...
                     "everywhere)");
  prm.declare_entry ("approximate projections","0",Patterns::Integer(),
  ...
  prm.declare_entry ("Cahn-Hilliard do Newton","1",Patterns::Integer(),
                     ...
                     "Eyre.");
  prm.declare_entry ("full nonlinear iteration","0",Patterns::Integer(),
                     "iterates between Navier-Stokes and concentration "
                     "if enabled");
  prm.declare_entry ("number reinit steps","2",Patterns::Integer(),
                     "number of iterations in reinitialization");
  prm.declare_entry ("number initial reinit steps","0",Patterns::Integer(),
                     "reinitialization steps before starting the time "
                     "loop (for bad initial profiles)");
  prm.declare_entry ("convection stabilization","0",Patterns::Integer(),
                                       ...
                                       ParameterHandler &prm)
{
  ...
  iterations_before_inner_solvers =
    prm.get_integer("lin its before inner solvers");
  prm.leave_subsection ();
  prm.leave_subsection ();
  ...
  AssertThrow (diffusion_length > 0,
               ExcMessage ("Diffusion length must be positive"));
  AssertThrow (epsilon > 0,
               ExcMessage ("Diffusion length must be positive"));
