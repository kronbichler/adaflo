...
NavierStokes<dim>::NavierStokes (const FlowParameters &parameters,
  ...
  this_mpi_process (Utilities::MPI::this_mpi_process(triangulation_in.get_communicator())),
  ...
  update_preconditioner_frequency (0)
{
  ...
  if (external_timer == 0)
    timer.reset (new TimerOutput (pcout, TimerOutput::summary,
...
template <int dim>
void NavierStokes<dim>::print_n_dofs () const
{
  ...
  pcout  << " Number of active cells: "
         ...
         << n_dofs.first + n_dofs.second << " ("
         << n_dofs.first << " + " << n_dofs.second << ")."
         << std::endl

         << " Approximate size last cell: "
         << min_cell_diameter/std::sqrt(dim)
         << std::endl;
}
...
template <int dim>
void NavierStokes<dim>::distribute_dofs()
{
...
template <int dim>
void NavierStokes<dim>::initialize_data_structures ()
{
  ...
  for (unsigned int d=0; d<dim; ++d)
    if (this->boundary->periodic_boundaries[d] !=
        std::pair<types::boundary_id,types::boundary_id>(-1, -1))
      {
        ...
        DoFTools::make_periodicity_constraints (dof_handler_u, in, out, d,
                                                hanging_node_constraints_u);
        DoFTools::make_periodicity_constraints (dof_handler_p, in, out, d,
                                                hanging_node_constraints_p);
      }
              ...
              const Function<dim> &)
{
  ...
  if (!time_stepping.at_end())
    {
      VectorTools::interpolate (this->mapping, get_dof_handler_u(),
...
template <int dim>
void NavierStokes<dim>::
initialize_matrix_free (MatrixFree<dim> *external_matrix_free)
{
  if (external_matrix_free != 0)
  ...
  else
    {
      ...
      matrix_free->reinit (this->mapping,
                           dof_handlers, constraints, quadratures,
                           data);
    }
  navier_stokes_matrix.initialize (*matrix_free, time_stepping,
...
template <int dim>
void NavierStokes<dim>::assemble_preconditioner ()
{
...
template <int dim>
void NavierStokes<dim>::build_preconditioner ()
{
...
template <int dim>
std::pair<unsigned int,double> NavierStokes<dim>::solve_system (const double linear_tolerance)
{
  if (parameters.linearization == FlowParameters::projection)
    return preconditioner.solve_projection_system(solution, solution_update, system_rhs,
  ...
  SolverControl solver_control_strong (std::max<int>((int)parameters.max_lin_iteration -
...
template <int dim>
void NavierStokes<dim>::init_time_advance (const bool print_time_info)
{
  Assert (system_is_setup == true,
          ExcMessage ("System has not yet been set up!"));
      ...
      if (time_stepping.step_no() > 1)
        {
          ...
          for (unsigned int i=0; i<n; ++i)
            {
              const double tmp = cur[i] -
                                 time_stepping.weight_old()/time_stepping.weight() * old[i] -
      ...
      if (parameters.rel_lin_iteration)
        {
          ...
          if (res * parameters.tol_lin_iteration < 0.5 * parameters.tol_nl_iteration ||
            ...
            linear_tolerance = std::min (parameters.tol_lin_iteration * res,
                                         parameters.tol_lin_iteration);
        }
      ...
      else if (parameters.output_verbosity == 2)
        {
          ...
          if (this_mpi_process == 0)
            std::printf ("    %-5.2e     %4d       %-5.2e\n",
                         update_norm, convergence.first, convergence.second);
        }
      else if (parameters.output_verbosity == 3)
        {
          ...
          if (this_mpi_process == 0)
            std::printf ("    %-5.2e   %-5.2e    %4d       %-5.2e\n",
                         incr_u, incr_p, convergence.first, convergence.second);
        }
  ...
  if (!this->boundary->pressure_fix.empty())
    {
      ...
      distance(std::numeric_limits<long int>::max(), 0.);
      QGauss<dim-1> quadrature(1);
      std::vector<double> pressure_values(1);
      FEFaceValues<dim> fe_values(this->mapping, fe_p, quadrature,
                                  update_values | update_quadrature_points);
      for (typename DoFHandler<dim>::active_cell_iterator cell=
             dof_handler_p.begin_active(); cell!=dof_handler_p.end(); ++cell)
                    ...
                    if (it->second.get() != 0)
                      {
                        ...
                        distance.second = it->second->value(fe_values.quadrature_point(0))
                                          - pressure_values[0];
                      }
                    else
...
end_loop:
      ...
      const double local_value = (distance.first == min_index ?
  ...
  if (solution.block(0).l2_norm() > 0)
    {
      ...
      const FlowParameters::PhysicalType physical_type =
      ...
      if (navier_stokes_matrix.use_variable_coefficients() == true)
        {
          ...
          for (unsigned int cell=0; cell<matrix_free->n_macro_cells(); ++cell)
            {
              VectorizedArray<double> *visc = navier_stokes_matrix.begin_viscosities(cell);
...
template <int dim>
void NavierStokes<dim>::apply_boundary_conditions()
{
  const double time = time_stepping.now();
  {
  ...
  if (!this->boundary->open_conditions_p.empty())
    {
      ...
      FEFaceValues<dim> fe_values (fe_u, face_quadrature,
      ...
      for (typename DoFHandler<dim>::active_cell_iterator cell =
             dof_handler_u.begin_active(); cell != dof_handler_u.end(); ++cell)
        for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face)
          if (cell->is_locally_owned())
            if (cell->at_boundary(face) &&
...
template <int dim>
void NavierStokes<dim>::refine_grid_pressure_based (const unsigned int max_grid_level,
...
template <int dim>
void NavierStokes<dim>::output_solution (const std::string output_name,
...
template <int dim>
std::size_t NavierStokes<dim>::memory_consumption () const
{
...
template <int dim>
void NavierStokes<dim>::print_memory_consumption (std::ostream &stream) const
{
  if (this_mpi_process == 0)
    {
      stream << "\n+--- Memory consumption Navier-Stokes objects ---\n"
             ...
             // << " MB\n"
             << "| Matrix-free objects: "
             << 1e-6*double(matrix_free->memory_consumption())
             << " MB\n"
             << "| Constraints: "
      ...
      stream << "| Vectors: "
             << 6e-6*double(system_rhs.memory_consumption())
             << " MB\n";
      stream << "| Memory consumption used objects:\n"
             << "| Triangulation: "
             << 1e-6*double(triangulation.memory_consumption()) << " MB\n"
             << "+------------------------------------------------\n";
    }
}
