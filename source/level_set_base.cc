  ...
  last_refine_step (0)
{
  ...
  {
    interpolation_concentration_pressure.reinit (this->navier_stokes.get_fe_p().dofs_per_cell,
    ...
    if (const FE_Q<dim> *fe_p = dynamic_cast<const FE_Q<dim>*>(&this->navier_stokes.get_fe_p()))
      {
    ...
    else if (const FE_Q_DG0<dim> *fe_p = dynamic_cast<const FE_Q_DG0<dim>*>(&this->navier_stokes.get_fe_p()))
      {
        const std::vector<unsigned int> lexicographic_p = fe_p->get_poly_space_numbering_inverse();
...
template <int dim>
void LevelSetBaseAlgorithm<dim>::setup_problem (const Function<dim> &initial_velocity_field,
...
template <int dim>
void LevelSetBaseAlgorithm<dim>::initialize_data_structures ()
{
...
template <int dim>
unsigned int LevelSetBaseAlgorithm<dim>::advance_time_step()
{
  ...
  if (this->parameters.do_iteration)
    {
      ...
      this->navier_stokes.get_matrix().old_times (this->navier_stokes.const_rhs, old_vectors);
      for ( ; step<20; ++step)
        {
          ...
          this->navier_stokes.get_matrix().residual (this->navier_stokes.system_rhs,
                                                     this->navier_stokes.solution,
...
template <int dim>
bool LevelSetBaseAlgorithm<dim>::mark_cells_for_refinement ()
{
  ...
  FEValues<dim> vel_values (this->navier_stokes.get_fe_u(), quadrature,
                            update_values);
    ...
    if (cell->is_locally_owned())      //Cell belongs to the local core or not
      {
        ...
        const double direction = 4.*this->time_stepping.step_size()*
                                 (ls_gradient*u_values[0])/ls_gradient.norm()/this->epsilon_used;
...
template <int dim>
void LevelSetBaseAlgorithm<dim>::output_solution (const std::string output_name,
                                                  const unsigned int n_subdivisions) const
{
  ...
  const FESystem<dim> joint_fe (this->navier_stokes.get_fe_u(), 1,
                                this->navier_stokes.get_fe_p(), 1,
                                *this->fe, 4,
                                *this->fe, dim);
  ...
  LinearAlgebra::distributed::Vector<double> joint_solution;
  {
    ...
    joint_solution.reinit (joint_dof_handler.locally_owned_dofs(),
                           locally_relevant_joint_dofs, this->triangulation.get_communicator());
  }


  {
    std::vector<types::global_dof_index> local_joint_dof_indices (joint_fe.dofs_per_cell);
    std::vector<types::global_dof_index> local_vel_dof_indices (this->navier_stokes.get_fe_u().dofs_per_cell);
    std::vector<types::global_dof_index> local_pre_dof_indices (this->navier_stokes.get_fe_p().dofs_per_cell);
    std::vector<types::global_dof_index> local_levelset_dof_indices (this->fe->dofs_per_cell);
      ...
      if (vel_cell->is_locally_owned())  //joint_cell->is_locally_owned()
        {
          ...
          for (unsigned int i=0; i<joint_fe.dofs_per_cell; ++i)
            {
              if (joint_dof_handler.locally_owned_dofs().is_element(local_joint_dof_indices[i]) == false)
                continue;
              ...
              if (joint_fe.system_to_base_index(i).first.first == 0)
                ...
                {
                  Assert (index < local_pre_dof_indices.size(),
                          ExcInternalError());
                  joint_solution(local_joint_dof_indices[i])
              ...
              else if (joint_fe.system_to_base_index(i).first.first == 3)
                {
                  ...
                  unsigned int cur_index = index%local_levelset_dof_indices.size();
                  joint_solution(local_joint_dof_indices[i])
                    = this->normal_vector_field.block(joint_fe.system_to_base_index(i).first.second)
  ...
  (dim+5+dim, DataComponentInterpretation::component_is_scalar);
  ...
  const unsigned int n_patches = n_subdivisions == 0 ?
                                 std::min(this->parameters.velocity_degree,
                                          this->parameters.concentration_subdivisions) : n_subdivisions;
