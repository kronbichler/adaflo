...
template <int dim>
NavierStokesMatrix<dim>::NavierStokesMatrix (const FlowParameters &parameters,
...
#define EXPAND_OPERATIONS(OPERATION)                                    \
  const unsigned int degree_p =                                         \
      matrix_free->get_dof_handler(1).get_fe().degree;                    \
  \
            ...
            const bool              pressure_average_fix)
{
  ...
  if (use_variable_coefficients == true)
    {
      ...
      variable_viscosities.resize (size, make_vectorized_array<double>(parameters.viscosity));
    }
  ...
  if (pressure_average_fix == true || parameters.augmented_taylor_hood)
    {
      parallel::distributed::Vector<double> pres_mass;
...
#define OPERATION(degree_p)                                             \
  matrix_free->cell_loop (&NavierStokesMatrix<dim>::template      \
                          local_pressure_mass_weight<degree_p>,   \
                          this, pres_mass, dummy)
      ...
      for (unsigned int mode=0; mode<2; ++mode)
        {
          ...
          for (std::vector<unsigned int>::const_iterator it =
               ...
               it != matrix_free->get_constrained_dofs(1).end(); ++it)
            pressure_constant_modes[mode].local_element(*it) = 0;

          inverse_pressure_average_weights[mode] = 1./(pressure_constant_modes[mode] *
                                                       pressure_constant_mode_weights[mode]);
        }
    }
}
...
void
NavierStokesMatrix<dim>
::apply_pressure_average_projection(parallel::distributed::Vector<double> &vector) const
{
...
template <int dim>
void
NavierStokesMatrix<dim>::vmult (parallel::distributed::BlockVector<double> &dst,
...
#define OPERATION(degree_p)                                             \
  matrix_free->cell_loop (&NavierStokesMatrix<dim>::template            \
                          local_operation<degree_p, parallel::distributed::BlockVector<double>, \
...
template <int dim>
void
NavierStokesMatrix<dim>::residual (parallel::distributed::BlockVector<double> &system_rhs,
...
template <int dim>
void
NavierStokesMatrix<dim>::divergence_vmult_add (parallel::distributed::Vector<double> &dst,
...
#define OPERATION(degree_p)                                             \
  matrix_free->cell_loop (&NavierStokesMatrix<dim>::template        \
                          local_divergence<degree_p,true>,          \
                          this, dst, src)
...
#define OPERATION(degree_p)                                             \
  matrix_free->cell_loop (&NavierStokesMatrix<dim>::template        \
                          local_divergence<degree_p,false>,         \
                          this, dst, src)
...
template <int dim>
void
NavierStokesMatrix<dim>::velocity_vmult (parallel::distributed::Vector<double> &dst,
...
template <int dim>
void
NavierStokesMatrix<dim>::pressure_poisson_vmult (parallel::distributed::Vector<double> &dst,
                                                 const parallel::distributed::Vector<double> &src) const
{
  Assert (matrix_free != 0, ExcNotInitialized());
  dst = 0;
...
template <int dim>
void
NavierStokesMatrix<dim>::pressure_mass_vmult (parallel::distributed::Vector<double> &dst,
                                              const parallel::distributed::Vector<double> &src) const
{
  Assert (matrix_free != 0, ExcNotInitialized());
  dst = 0;
...
#undef OPERATION
  ...
  if (pressure_constant_mode_weights[1].size() > 0 &&
      parameters.linearization != FlowParameters::projection)
    {
      ...
      dst.add(-weight * inverse_pressure_average_weights[1],
              pressure_constant_modes[1]);
    }
}
...
template <int dim>
void
NavierStokesMatrix<dim>::pressure_convdiff_vmult (parallel::distributed::Vector<double> &dst,
                                                  const parallel::distributed::Vector<double> &src) const
{
  Assert (matrix_free != 0, ExcNotInitialized());
  dst = 0;
...
#define OPERATION(degree_p)                                             \
  matrix_free->cell_loop (&NavierStokesMatrix<dim>::template            \
                          local_pressure_convdiff<degree_p>,            \
                          this, dst, src)
...
namespace
{
  ...
  inline
  void get_velocity_values (FEEval &fe_eval,
                            const parallel::distributed::BlockVector<double> &vec)
  {
  ...
  template <typename FEEval>
  inline
  void get_velocity_values_plain (FEEval &fe_eval,
                                  const parallel::distributed::BlockVector<double> &vec)
  {
  ...
  inline
  void get_velocity_values (FEEval &fe_eval,
                            const parallel::distributed::Vector<double> &vec)
  {
  ...
  template <typename FEEval>
  inline
  void get_pressure_values (FEEval &fe_eval,
                            const parallel::distributed::BlockVector<double> &vec)
  {
  ...
  template <typename FEEval>
  inline
  void get_pressure_values_plain (FEEval &fe_eval,
                                  const parallel::distributed::BlockVector<double> &vec)
  {
  ...
  template <typename FEEval>
  inline
  void distribute_velocity_ltg (FEEval &fe_eval,
                                parallel::distributed::BlockVector<double> &vec)
  {
  ...
  template <typename FEEval>
  inline
  void distribute_pressure_ltg (FEEval &fe_eval,
                                parallel::distributed::BlockVector<double> &vec)
  {
                 ...
                 const std::pair<unsigned int,unsigned int> &cell_range) const
{
  ...
  const vector_t time_step_weight = make_vectorized_array<double>(parameters.physical_type == FlowParameters::incompressible ? time_stepping->weight() : 0.);
  const vector_t time_step_weight_old = make_vectorized_array<double>(time_stepping->weight_old());
  const vector_t time_step_weight_old_old = make_vectorized_array<double>(time_stepping->weight_old_old());
  ...
  velocity_stored *linearized =
    ...
    &linearized_velocities[cell_range.first*velocity.n_q_points] : 0;

  const bool use_variable_coefficients = variable_densities.size() > 0;
  const vector_t *rho_values = use_variable_coefficients ?
                               begin_densities(cell_range.first) : 0;
  const vector_t *mu_values = use_variable_coefficients ?
                              begin_viscosities(cell_range.first) : 0;
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      if (LocalOps == NavierStokesOps::residual ||
          LocalOps == NavierStokesOps::vmult)
        {
          if (LocalOps == NavierStokesOps::residual)
      ...
      for (unsigned int q=0; q<velocity.n_q_points; ++q)
        {
          ...
          if (parameters.physical_type != FlowParameters::stokes)
            {
              ...
              const vector_t rho = use_variable_coefficients ?
                                   rho_values[q] : make_vectorized_array<double>(parameters.density);
                  ...
                  if (need_extrapolated_velocity)
                    {
                      ...
                      for (unsigned int d=0; d<dim; ++d)
                        old_val[d] = time_stepping->extrapolate(old_val[d],
                                                                old_old_val[d]);
                      if (parameters.linearization == FlowParameters::coupled_velocity_explicit)
                        for (unsigned int d=0; d<dim; ++d)
                          {
                            vector_t res = conservative_form * extrapol_divergence *
                                           old_val[d];
                            for (unsigned int e=0; e<dim; ++e)
              ...
              // matrix-vector cases
              else if (parameters.linearization == FlowParameters::coupled_implicit_newton)
                {
                  vector_t factor1 = conservative_form * divergence;
                  vector_t factor2 = conservative_form * trace(linearized[q].second);
                  for (unsigned int d=0; d<dim; ++d)
                    {
                      vector_t res = factor1 * linearized[q].first[d] + factor2 *val_u[d];
                      for (unsigned int e=0; e<dim; ++e)
              ...
              else if (parameters.linearization != FlowParameters::coupled_velocity_explicit)
                for (unsigned int d=0; d<dim; ++d)
                  {
                    vector_t res = conservative_form * linearized[q].second[0][0] * val_u[d];
                    for (unsigned int e=0; e<dim; ++e)
                      res += linearized[q].first[e] * grad_u[d][e];
                    conv[d] += tau1 * res;
                  }
          ...
          const vector_t tmu = (use_variable_coefficients ?
                                mu_values[q] : make_vectorized_array<double>(parameters.viscosity)) * tau1;
          ...
          for (unsigned int d=0; d<dim; ++d)
            {
              grad_u[d][d] = tmu_times_2 * grad_u[d][d] +
                             parameters.tau_grad_div * divergence;
                  ...
                  const std::pair<unsigned int,unsigned int>  &cell_range) const
{
  ...
  const VectorizedArray<double> *mu_values = variable_viscosities.empty() ?
                                             0 : begin_viscosities(cell_range.first);
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      for (unsigned int q=0; q<velocity.n_q_points; ++q)
        {
          VectorizedArray<double> weight = weight_by_viscosity ?
                                           (variable_viscosities.empty() ?
                                            make_vectorized_array<double>(-parameters.viscosity) : -mu_values[q])
                                           :
      ...
      for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
        {
          ...
          for (unsigned int q=0; q<pressure.n_q_points; ++q)
            pressure.submit_gradient(pressure.get_gradient(q)*(1./(time_stepping->weight()*begin_densities(cell)[q])), q);
      ...
      for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
        {
          ...
          const vector_t rho_value = use_variable_coefficients ?
                                     begin_densities(cell)[data.get_n_q_points(0)/2] :
                                     make_vectorized_array(std::min(parameters.density,
                                                                    parameters.density + parameters.density_diff));
          const vector_t coefficient =
            parameters.physical_type == FlowParameters::incompressible_stationary ?
            make_vectorized_array<double>(1.) : 1./(time_stepping->weight()*rho_value);
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      const vector_t coefficient =
        (parameters.linearization == FlowParameters::projection ||
         parameters.physical_type == FlowParameters::incompressible_stationary) ?
        make_vectorized_array<double>(1.) : 1./(mu_value+parameters.tau_grad_div);
...
void
NavierStokesMatrix<dim>::
local_pressure_mass_weight (const MatrixFree<dim> &data,
                            parallel::distributed::Vector<double>      &dst,
                            const unsigned int &,
                            const std::pair<unsigned int,unsigned int> &cell_range) const
{
