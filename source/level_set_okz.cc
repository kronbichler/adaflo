...
template <int dim>
void LevelSetOKZSolver<dim>::initialize_data_structures ()
{
  ...
  parallel::distributed::Vector<double> diagonal(this->solution_update.block(0));
  {
    ...
    FEValues<dim> fe_values(this->mapping, *this->fe, quadrature,
      ...
      if (cell->is_locally_owned())
        {
          ...
          .distribute_local_to_global(local_rhs, local_dof_indices, diagonal);
        }
  ...
  //                          this->constraints,
  //                          QIterated<1> (QGauss<1>(2), this->parameters.concentration_subdivisions),
  //                          data);
  ...
  }
  {
    ...
    csp.reinit (this->dof_handler.locally_owned_dofs(),
                ...
                relevant_dofs, this->triangulation.get_communicator());
    std::vector<types::global_dof_index> local_dof_indices(this->fe->dofs_per_cell);
    ...
    for ( ; cell != endc; ++cell)
      if (cell->is_locally_owned())
        {
          cell->get_dof_indices (local_dof_indices);
          this->constraints_normals.add_entries_local_to_global(local_dof_indices, csp,
                                                                false, dof_mask);
        }
    csp.compress();
    projection_matrix.reset(new BlockMatrixExtension());
  ...
  }
  {
    ...
    ilu_projection_matrix.reset(new BlockILUExtension());
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      const VectorizedArray<double> damping =
        4.*Utilities::fixed_power<2>(std::max(min_diameter,
                                              this->cell_diameters[cell]/
                                              static_cast<double>(ls_degree)));
      ...
      for (unsigned int v=0; v<data.n_components_filled(cell); ++v)
        {
          typename DoFHandler<dim>::active_cell_iterator dcell =this->matrix_free.get_cell_iterator(cell, v, 2);
          dcell->get_dof_indices(scratch.dof_indices);
          this->constraints_normals.distribute_local_to_global(scratch.matrices[v],
                                                               scratch.dof_indices,
                                             ...
                                             const std::pair<unsigned int,unsigned int> &cell_range)
{
  ...
  //3 means constraints_curvature and so an
      ...
      if (use_variable_parameters)
        {
          ...
          vector_t *viscosities = this->navier_stokes.get_matrix().begin_viscosities(cell);
      ...
      for (unsigned int q=0; q<curv_values.n_q_points; ++q)
        {
          ...
          Tensor<1,dim,vector_t> force =
            ...
            (this->parameters.interpolate_grad_onto_pressure ?
             pre_values.get_gradient(q) : ls_values.get_gradient(q));

          // gravity
          vector_t actual_rho = use_variable_parameters ?
      ...
      for (unsigned int q=0; q<ls_values.n_q_points; ++q)
        {
          ...
          const Tensor<1,dim,VectorizedArray<double> > ls_grad = ls_values.get_gradient(q);
          ls_values.submit_value(ls_val * this->time_stepping.weight() +
                                 ls_grad * velocities[q], q);
...
{
  //The second input argument below refers to which constrains should be used, 2 means constraints (for LS-function)
  //and 0 means &navier_stokes.get_constraints_u()
  FEEvaluation<dim,ls_degree,2*ls_degree,1> ls_values(data,2,2);
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      vel_values_old_old.read_dof_values_plain(this->navier_stokes.solution_old_old.block(0));
      ...
      if (this->parameters.convection_stabilization)
        {
          ...
          for (unsigned int q=0; q<ls_values.n_q_points; ++q)
            {
              ...
              Tensor<1,dim,vector_t> u = (vel_values_old.get_value(q)+
                                          vel_values_old_old.get_value(q));
              vector_t dc_dt = (ls_values_old.get_value(q)-ls_values_old_old.get_value(q))/this->time_stepping.old_step_size();
              vector_t residual = std::abs(dc_dt + u * (ls_values_old.get_gradient(q)+
                                                        ls_values_old_old.get_gradient(q)) * 0.25);
          ...
          artificial_viscosities[cell] = 0.03 * max_velocity * cell_diameter *
      ...
      for (unsigned int q=0; q<ls_values.n_q_points; ++q)
        {
          ...
          vector_t old_value = this->time_stepping.weight_old()*
                               ls_values_old.get_value(q);
          if (this->time_stepping.scheme() == TimeStepping::bdf_2 &&
              this->time_stepping.step_no() > 1)
            old_value += this->time_stepping.weight_old_old() *
                         ls_values_old_old.get_value(q);
          ...
          vector_t residual = -(ls_val * this->time_stepping.weight()
                                +
                                vel_values.get_value(q) * ls_grad
                                +
                                old_value);
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      const VectorizedArray<Number> damping =
        Number(4.)*Utilities::fixed_power<2>(std::max(min_diameter,
                                                      cell_diameters[cell]/
                                                      static_cast<Number>(ls_degree)));
      for (unsigned int q=0; q<phi.n_q_points; ++q)
        {
          phi.submit_value(phi.get_value(q), q);
          phi.submit_gradient(phi.get_gradient(q) * damping, q);
...
{
  //The second input argument below refers to which constrains should be used, 4 means constraints_normals
  //and 2 means constraints (for LS-function)
  ...
  for (unsigned int cell=cell_range.first; cell<cell_range.second; ++cell)
    {
      ...
      const VectorizedArray<double> damping =
        ...
        Utilities::fixed_power<2>(std::max(min_diameter,
                                           this->cell_diameters[cell]/
                                           static_cast<double>(ls_degree)))
        :
                                            ...
                                            const std::pair<unsigned int,unsigned int> &cell_range) const
{
  ...
  const double dtau_inv = std::max(0.95/(1./(dim*dim)*this->minimal_edge_length/this->parameters.concentration_subdivisions),
...
{
  //The second input argument below refers to which constrains should be used, 2 means constraints (for LS-function)
  //and 4 means constraints_normals
  FEEvaluation<dim,ls_degree,2*ls_degree> phi(data, 2, 2);
        ...
        if (!diffuse_only)
          {
            ...
            phi.submit_gradient(normal * (0.5 * (1. - phi.get_value(q)*
                                                 phi.get_value(q)) -
                                          (normal * grad * diffusion)), q);
          }
        else
          {
...
template <int dim>
void LevelSetOKZSolver<dim>::compute_force ()
{
  compute_heaviside();
  compute_curvature();
...
#define OPERATION(ls_degree,vel_degree)                                 \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_force<ls_degree,vel_degree>, \
                               this, this->navier_stokes.user_rhs.block(0), this->solution.block(0))
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template            \
                               local_advance_concentration<c_degree,u_degree>, \
                               this, dst, src)
...
#undef OPERATION
  ...
  if (this->parameters.convection_stabilization)
    {
      ...
      FEFaceValues< dim > fe_face_values (*this->fe, this->matrix_free.get_face_quadrature(1),
      ...
      for (unsigned int mcell=0; mcell<this->matrix_free.n_macro_cells(); ++mcell)
        for (unsigned int v=0; v<this->matrix_free.n_components_filled(mcell); ++v)
          {
            typename DoFHandler<dim>::active_cell_iterator cell =this->matrix_free.get_cell_iterator(mcell, v, 2);
            cell_rhs = 0;
                ...
                if (cell->face(face)->at_boundary())
                  {
                    if (this->boundary->symmetry.find(cell->face(face)->boundary_id()) ==
                        this->boundary->symmetry.end())
                      {
                        ...
                        for (unsigned int i=0; i<this->fe->dofs_per_cell; ++i)
                          {
                            for (unsigned int q=0; q<fe_face_values.get_quadrature().size(); ++q)
                              {
            ...
            cell->get_dof_indices (local_dof_indices);
            this->constraints.distribute_local_to_global(cell_rhs, local_dof_indices, dst);
  ...
  for (unsigned int i=0; i<this->matrix_free.get_constrained_dofs(2).size(); ++i)
    dst.local_element(this->matrix_free.get_constrained_dofs(2)[i]) =
      preconditioner.get_vector().local_element(this->matrix_free.get_constrained_dofs(2)[i]) *
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_normal<c_degree,double>,   \
                               this, dst, src)
  ...
  for (unsigned int i=0; i<this->matrix_free.get_constrained_dofs(4).size(); ++i)
    for (unsigned int d=0; d<dim; ++d)
      dst.block(d).local_element(this->matrix_free.get_constrained_dofs(4)[i]) =
        preconditioner.get_vector().local_element(this->matrix_free.get_constrained_dofs(4)[i]) *
                                             ...
                                             const parallel::distributed::BlockVector<float> &src) const
{
  dst = 0.;
#define OPERATION(c_degree,u_degree)                                    \
  matrix_free_float.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_normal<c_degree,float>,    \
                               this, dst, src)
                                                ...
                                                const bool apply_diffusion) const
{
  ...
  if (apply_diffusion)
    {
      //diffusion_setting will be 1 (true) in local_compute_curvature so that damping will be added
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_curvature<c_degree,1>,    \
                               this, dst, src)
...
#undef OPERATION
    ...
    {
      //diffusion_setting will be 0 (fals) in local_compute_curvature so that NO damping will be added
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_curvature<c_degree,0>,   \
                               this, dst, src)
...
#undef OPERATION
  ...
  for (unsigned int i=0; i<this->matrix_free.get_constrained_dofs(3).size(); ++i)
    dst.local_element(this->matrix_free.get_constrained_dofs(3)[i]) =
      preconditioner.get_vector().local_element(this->matrix_free.get_constrained_dofs(3)[i]) *
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_reinitialize<c_degree,true>,         \
                               this, dst, src)
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_reinitialize<c_degree,false>,        \
                               this, dst, src)
...
{
  ReinitializationMatrix (const LevelSetOKZSolver<dim> &problem,
...
LevelSetOKZSolver<dim>::advance_concentration ()
{
  ...
  // apply boundary values
  {
    ...
    VectorTools::interpolate_boundary_values (this->mapping, this->dof_handler,
                                              dirichlet,
                                              boundary_values);
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_advance_concentration_rhs<c_degree,u_degree>, \
                               this, rhs, this->solution.block(0))
...
#undef OPERATION
  ...
  if (this->parameters.convection_stabilization)
    {
      ...
      FEFaceValues< dim > fe_face_values (this->mapping,
                                          *this->fe, this->matrix_free.get_face_quadrature(1),
                                          update_values |  update_gradients |
                                          update_JxW_values | update_normal_vectors);
                ...
                if (cell->face(face)->at_boundary())
                  {
                    if (this->boundary->symmetry.find(cell->face(face)->boundary_id()) ==
                      ...
                      {
                        fe_face_values.reinit (cell, face);
                        fe_face_values.get_function_gradients(this->solution.block(0), local_gradients);
  ...
  if (!this->parameters.do_iteration)
    this->pcout << "  Concentration advance: advect ["
                << initial_residual << "/" << n_iterations << "]";
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_normal_rhs<c_degree>,      \
                               this, this->normal_vector_rhs, this->solution.block(0))
  ...
  if (this->parameters.approximate_projections == true)
      ...
      for (unsigned int i=0; i<n_steps; ++i)
        {
          for (unsigned int block=0; block<dim; ++block)
            {
              this->constraints_normals.distribute(this->normal_vector_field.block(block));
              compute_curvature_vmult(this->normal_vector_rhs.block(block),
                                      this->normal_vector_field.block(block), 2);
            }
  ...
  else
    {
      ...
      solver.solve (*projection_matrix, this->normal_vector_field,
                    this->normal_vector_rhs, *ilu_projection_matrix);
      //this->pcout << "N its normal: " << solver_control.last_step() << std::endl;
    }
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template        \
                               local_compute_curvature_rhs<c_degree>,   \
                               this, rhs, this->solution.block(0))
        ...
        if (this->solution.block(1).local_element(i) > 1e-4)
          {
            ...
            const double distance = (1-c_val*c_val) > 1e-2 ?
                                    this->epsilon_used *
                                    std::log((1.+c_val)/(1.-c_val)) : 0;

            this->solution.block(1).local_element(i) = 1./(1./this->solution.block(1).local_element(i)+distance/(dim-1));
          }
    }
...
// elements now
template <int dim>
void LevelSetOKZSolver<dim>::compute_heaviside ()
{
  ...
  endc = this->dof_handler.end();
  for ( ; cell!=endc; ++cell)
    if (cell->is_locally_owned())  //Check if the cell is owned by the local processor
      {
        cell->get_dof_indices (local_dof_indices);
            ...
            for (unsigned int i=0; i<dofs_per_cell; ++i)
              {
                ...
                // -2 and 2.
                distance *= this->parameters.epsilon * 2./this->parameters.concentration_subdivisions;
...
template <int dim>
void LevelSetOKZSolver<dim>::reinitialize (const unsigned int stab_steps,
                                           ...
                                           const bool)
{
  ...
  if (this->last_concentration_range.first < -1.02 ||
    ...
    actual_diff_steps += 3;
  if (!this->parameters.do_iteration)
    this->pcout << (this->time_stepping.now()==this->time_stepping.start() ? "  " : " and ")
                << "reinitialize (";
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template \
                               local_reinitialize_rhs<c_degree,true>, \
                               this, rhs, this->solution.block(0))
...
#define OPERATION(c_degree,u_degree)                                    \
  this->matrix_free.cell_loop (&LevelSetOKZSolver<dim>::template \
                               local_reinitialize_rhs<c_degree,false>, \
                               this, rhs, this->solution.block(0))
