...
template <int dim>
void
BoundaryVelocity<dim>::vector_value (const Point<dim> &p,
                                     Vector<double>   &values) const
{
  ...
  if (std::abs(p[1]-1.) < 1e-12)
    {
      values(0) = 0.25*(1-std::cos(numbers::PI*2*p[0])) * (1-std::cos(numbers::PI*2*p[2]/3.));
    }
}
...
template <int dim>
void LidDrivenCavityProblem<dim>::output_results () const
{
  ...
  std::vector<Point<dim-1> > evaluate_points(navier_stokes.get_parameters().velocity_degree*3+4);
  ...
  FEFaceValues<dim> fe_face_values(navier_stokes.get_dof_handler_u().get_fe(), quadrature,
  ...
  std::string filename = navier_stokes.get_parameters().output_filename +
  ...
  filename += "-Q" + Utilities::int_to_string(navier_stokes.get_parameters().velocity_degree) +
              "-statistics.dat";
  std::cout << std::endl << std::endl << "filename: " << filename << std::endl << std::endl << std::endl;
...
template <int dim>
void LidDrivenCavityProblem<dim>::perform_data_exchange(std::vector<double> &positions,
                                                        std::vector<Tensor<1,dim> > &velocities) const
{
...
template <int dim>
void LidDrivenCavityProblem<dim>::run ()
{
  ...
  unsigned int adaptive_refinements = navier_stokes.get_parameters().adaptive_refinements+1;
