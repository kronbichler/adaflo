...
namespace MicroFluidic
{
   ...
   *  and its members. We will define the geometry in
   * create_geometry_and_partition_boundary () and define the boundary indicator. We use
   * these latter ones to specify different boundary conditions. Through the member
   * functions set_all_boundary_conditions () we impose the boundary conditions. We remind
   * that they are defined in the file boundary_condition.h. In  member function run (),
   * we will call functions defined in TwoPhase.h for the resolution.
   *
   * */
  ...
  public:
    ...
    void
    run(const TwoPhaseParameters &parameters);
  ...
  private:
    ...
    void
    set_all_boundary_conditions(const TwoPhaseParameters &parameters);
  };
  ...
  */
      ...
      for (unsigned int face = 0; face < GeometryInfo<dim>::faces_per_cell; ++face)
        {
          ...
          if (cell->face(face)->at_boundary())
            {
              if ((face_center[1] <= 1.5 && face_center[1] >= 0.5) &&
              ...
              if ((face_center[1] <= 4.5 && face_center[1] >= 3.5) &&
                  (face_center[0] <= 2 && face_center[0] >= 1))
  ...
  void create_triangulation(Triangulation<3> &triangulation)
  {
    ...
    set_boundary_indicators(triangulation);
    const CylindricalManifold<3> boundary_description_12(
      Point<3>(0, 0, 1), Point<3>(-1, 0.5, 0)); // Point 20
    triangulation.set_manifold(12, boundary_description_12);

    const CylindricalManifold<3> boundary_description_23(Point<3>(0, 0, 1),
                                                         Point<3>(2, 0.5, 0)); // Point 17
    triangulation.set_manifold(23, boundary_description_23);


    const CylindricalManifold<3> boundary_description_34(Point<3>(0, 0, 1),
                                                         Point<3>(2, 4.5, 0)); // Point 18
    triangulation.set_manifold(34, boundary_description_34);

    const CylindricalManifold<3> boundary_description_41(
      Point<3>(0, 0, 1), Point<3>(-1, 4.5, 0)); // Point 19
    triangulation.set_manifold(41, boundary_description_41);
  }
  ...
  template <int dim>
  void
  MicroFluidicProblem<dim>::set_all_boundary_conditions(
    const TwoPhaseParameters &parameters)
  {
    ...
    solver->set_velocity_dirichlet_boundary(
      1, std::make_shared<BoundaryValuesVelocity<dim>>(1), -1);
    solver->set_velocity_dirichlet_boundary(
      2, std::make_shared<BoundaryValuesVelocity<dim>>(2), -1);
    solver->set_velocity_dirichlet_boundary(
      3, std::make_shared<BoundaryValuesVelocity<dim>>(3), -1);
    ...
    */
    // set_open_boundary_with_normal_flux is defined in boundary_description.h (cf
    // Navier-Stokes file)
    solver->set_open_boundary_with_normal_flux(
      4, std::make_shared<Functions::ConstantFunction<dim>>(50, 1));
  }
  ...
  template <int dim>
  void
  MicroFluidicProblem<dim>::run(const TwoPhaseParameters &parameters)
  {
    // Print the finite element pair
    pcout << std::endl;
    pcout
      << "  ****************** Finite Element pair used in this computation *************"
      << std::endl;
    pcout
      << "  *****                                                                   *****"
      << std::endl;
    ...
    if (parameters.augmented_taylor_hood == 1)
      pcout
        << "  *****              The Augmented Taylor-Hood element is used.           *****"
        << std::endl;
    else
      pcout
        << "  *****              The Taylor-Hood element is used.                     *****"
        << std::endl;
    pcout << "  *****              Number of MPI processes: " << std::setw(5)
          << Utilities::MPI::n_mpi_processes(mpi_communicator)
          << "                       *****" << std::endl;
    pcout << "  *****              Number of threads:       " << std::setw(5)
    ...
    solver->setup_problem(Functions::ConstantFunction<dim>(0., dim),
                          InitialValuesLS<dim>());
    ...
    while (solver->get_time_stepping().at_end() == false)
      {
        ...
        if (solution_data.size() > 0 &&
          ...
          {
            const int time_step =
              1.000001e4 * solver->get_time_stepping().max_step_size();
