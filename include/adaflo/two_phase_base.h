...
public:
  ...
  virtual void init_time_advance();
  ...
  std::vector<double>
  compute_bubble_statistics(std::vector<Tensor<2,dim> > *interface_points = 0,
  ...
  std::vector<double>
  compute_bubble_statistics_immersed(std::vector<Tensor<2,dim> > *interface_points = 0) const;

  void set_adaptive_time_step(const double velocity_norm) const;

  double get_maximal_velocity() const;
  std::pair<double,double> get_concentration_range () const;
  ...
  const NavierStokes<dim> &get_navier_stokes() const
  {
    return navier_stokes;
  }

  NavierStokes<dim> &get_navier_stokes()
  {
    return navier_stokes;
  }
