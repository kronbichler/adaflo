...
public:
  ...
  virtual void output_solution (const std::string output_name,
...
protected:
  ...
  virtual void evaluate_heaviside_function (FEValues<dim> &fe_values, std::vector<double> &cell_heaviside,
  ...
  // advection step
  virtual void advance_concentration () = 0;

  virtual void compute_normal (const bool fast_computation) = 0;

  virtual void compute_curvature (const bool diffuse_large_values = false) = 0;
