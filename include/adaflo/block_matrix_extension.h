...
public:
              ...
              const LinearAlgebra::distributed::BlockVector<double> &src) const
  {
    ...
    Epetra_MultiVector tril_dst (View, prec.OperatorRangeMap(), dst_ptrs, dst.n_blocks());
    Epetra_MultiVector tril_src (View, prec.OperatorDomainMap(), src_ptrs, src.n_blocks());
...
public:
                   ...
                   const std::vector<unsigned int> &index_by_component)
  {
    ...
    src_cpy.reset(new Epetra_MultiVector(matrix.trilinos_matrix().DomainMap(),
                                         dim));
    dst_cpy.reset(new Epetra_MultiVector(matrix.trilinos_matrix().RangeMap(),
                                         dim));
  }
                 ...
                 const LinearAlgebra::distributed::Vector<double> &src) const
  {
    ...
    AssertDimension (size_type(dst.end() - dst.begin()),
                     static_cast<size_type>(prec.OperatorRangeMap().NumMyPoints())*dim);
    AssertDimension (size_type(src.end() - src.begin()),
                     static_cast<size_type>(prec.OperatorDomainMap().NumMyPoints())*dim);
