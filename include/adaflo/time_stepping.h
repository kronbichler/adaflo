...
public:
  ...
  double tau2 () const;
  double weight () const;
  double max_weight_uniform () const;
  double weight_old () const;
  double weight_old_old () const;
  bool weight_has_changed () const;
  ...
  bool at_tick(const double tick_size) const;
...
template <typename Number>
inline Number
TimeStepping::extrapolate(const Number &old,
