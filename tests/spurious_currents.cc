...
MicroFluidicProblem<dim>::evaluate_spurious_velocities()
{
  ...
  double pressure_jump = 0;
  {
    ...
    FEValues<dim>     ns_values(navier_stokes.get_fe_p(),
                            quadrature_formula,
      ...
      if (ns_cell->is_locally_owned())
        {
          ...
          if (ns_cell->center().norm() < 0.1)
            {
              ns_values.get_function_values(navier_stokes.solution.block(1), p_values);
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
  ...
  while (solver->get_time_stepping().at_end() == false)
    {
      ...
      if (solution_data.size() > 0 &&
          ...
          solver->get_time_stepping().at_tick(parameters.output_frequency))
        {
          ...
          filename3 << Utilities::int_to_string((int)(100. / parameters.surface_tension),
                                                4)
                    << "-";
