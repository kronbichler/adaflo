                                   ...
                                   Vector<double>   &values) const
{
  ...
  switch (dim)
    {
    case 3:
      values(0) = -a*(std::exp(a*p[0])*std::sin(a*p[1]+d*p[2]) +
                      std::exp(a*p[2])*std::cos(a*p[0]+d*p[1]))*
                  std::exp(-nu*d*d*time);
      values(1) = -a*(std::exp(a*p[1])*std::sin(a*p[2]+d*p[0]) +
                      std::exp(a*p[0])*std::cos(a*p[1]+d*p[2]))*
                  std::exp(-nu*d*d*time);
      values(2) = -a*(std::exp(a*p[2])*std::sin(a*p[0]+d*p[1]) +
...
{
public:
  ExactSolutionP (const double viscosity = 1.,
                  const double time = 0.)
    :
    Function<dim>(1, time),
    nu (viscosity)
...
template <int dim>
void BeltramiProblem<dim>::compute_errors () const
{
  ...
  FEValues<dim> fe_values(navier_stokes.get_fe_u(), quadrature_div,
  ...
  for (typename DoFHandler<dim>::active_cell_iterator cell =
       ...
       cell != navier_stokes.get_dof_handler_u().end(); ++cell, ++index)
  ...
  const double cell_div = std::sqrt(Utilities::MPI::sum(cellwise_errors.norm_sqr(),
                                                        MPI_COMM_WORLD));
  ...
  const double p_l2_error = std::sqrt(Utilities::MPI::sum(p_l2_error_loc,
                                                          MPI_COMM_WORLD));
  ...
  const double u_l2_error = std::sqrt(Utilities::MPI::sum(u_l2_error_loc,
                                                          MPI_COMM_WORLD));
  ...
  pcout << "  L2-Errors relative: ||e_p||_L2 = " << p_l2_error/p_l2_norm
        << ",   ||e_u||_L2 = " << u_l2_error/u_l2_norm
        << std::endl;
  pcout << "  Cell divergence:    |div(u)|_cells = " << cell_div
        << std::endl;
...
template <int dim>
void BeltramiProblem<dim>::output_results () const
{
  ...
  VectorTools::interpolate (navier_stokes.get_dof_handler_u(),
                            ExactSolutionU<dim> (nu, time),
                            exact.block(0));
  navier_stokes.interpolate_pressure_field(ExactSolutionP<dim>(nu, time),
                                           exact.block(1));
...
void BeltramiProblem<dim>::run ()
{
  ...
  if (navier_stokes.time_stepping.at_tick(navier_stokes.get_parameters().output_frequency) == false)
    compute_errors ();
}
