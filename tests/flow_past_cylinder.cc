...
FlowPastCylinder<dim>::FlowPastCylinder(const FlowParameters &parameters)
  ...
  , cylinder_manifold(
      dim == 2 ? static_cast<Manifold<dim> *>(new PolarManifold<dim>(get_center<dim>())) :
                 reinterpret_cast<Manifold<dim> *>(
                   new CylindricalManifold<3>(get_direction<3>(), get_center<3>())))
...
create_triangulation(Triangulation<2> &tria, const bool compute_in_2d = true)
{
  ...
  GridGenerator::subdivided_hyper_rectangle(
    left, std::vector<unsigned int>({3U, 4U}), Point<2>(), Point<2>(0.3, 0.41), false);
  GridGenerator::subdivided_hyper_rectangle(right,
  ...
  for (Triangulation<2>::cell_iterator cell = middle.begin(); cell != middle.end();
       ++cell)
...
FlowPastCylinder<dim>::run()
{
  ...
  navier_stokes.set_velocity_dirichlet_boundary(
    1, std::make_shared<InflowVelocity<dim>>(0., false));

  navier_stokes.set_open_boundary(2, std::make_shared<Functions::ZeroFunction<dim>>(1));
  timer.leave_subsection();
